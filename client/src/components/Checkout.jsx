...
export default function Checkout({ booking, setBooking, setStep, user }) {
  ...
  useEffect(() => {
    if (
      ...
      email
        .split("@")[1]
        .split(".")
        .every((text) => text) // is there text before & after every . after @
    ) {
      setEmailValid(true);
    } else {
      setEmailValid(false);
  ...
  const pay = () => {
    ...
    const bookingExtras = Object.keys(extras).map((key) => {
      return { name: key, quantity: extras[key] };
    });
  ...
  return (
    <>
      <Page>
        ...
        <CardPageStyle>
          <label onClick={pay} htmlFor="email" style={{ color: "#144c74" }}>
